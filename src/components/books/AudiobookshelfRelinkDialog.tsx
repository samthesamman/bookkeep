...
import { audiobookshelfApi, hardcoverApi } from '@/lib/api';
import { transformHardcoverBook, type HardcoverBook } from '@/lib/hardcover';

/**
 * Search Hardcover and move the Audiobookshelf item currently linked to this
 * book onto the chosen book instead. Sibling of CalibreRelinkDialog.
 */
export function AudiobookshelfRelinkDialog({
  fromHardcoverId,
  open,
  onOpenChange,
  onLinked,
}: {
  fromHardcoverId: number;
  open: boolean;
  ...
  const handleLink = async (hardcoverId: number) => {
    ...
    try {
      await audiobookshelfApi.relink(fromHardcoverId, hardcoverId);
      toast.success('Audiobook re-linked', {
        description: 'The Audiobookshelf item now points at the chosen book.',
      });
      onLinked();
      onOpenChange(false);
    } catch (error) {
      toast.error('Failed to re-link audiobook', {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Link this audiobook to a Hardcover book</DialogTitle>
        </DialogHeader>
