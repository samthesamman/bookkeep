...
import { booksApi, hardcoverApi } from '@/lib/api';
import { transformHardcoverBook, type HardcoverBook } from '@/lib/hardcover';

/** Search Hardcover and manually point a Book row at the chosen match. */
export function LinkHardcoverDialog({
  bookId,
  initialQuery,
  open,
  onOpenChange,
  ...
  const { data, isFetching } = useQuery({
    queryKey: ['hardcover-manual-link-search', query],
  ...
  const handleLink = async (hardcoverId: number) => {
    ...
    try {
      await booksApi.linkHardcover(bookId, hardcoverId);
      toast.success('Book linked', { description: 'Metadata pulled in from Hardcover.' });
      onLinked();
      onOpenChange(false);
    } catch (error) {
